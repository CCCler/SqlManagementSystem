...
(async()=>{
  ...
  assert.match(await page.locator('#results').innerText(),/仅展示，不执行/);
  assert.match(await page.locator('#results').innerText(),/Join/);
  const multiplied = await run('UPDATE t SET age=age*2;');
  assert.equal(multiplied.results[0].affected_rows,1);
  assert.equal((await run('SELECT * FROM t;')).results[0].rows[0][1],42);
  await run("CREATE USER admin IDENTIFIED BY 'browser-old';");
  await page.locator('#login-button').click();
  await page.locator('#login-user').fill('admin');
  await page.locator('#login-password').fill('browser-old');
  await page.locator('#login-form button[type="submit"]').click();
  await page.waitForFunction(()=>document.querySelector('#login-button').textContent.includes('admin'));
  await run("ALTER USER admin IDENTIFIED BY 'browser-new';");
  assert.equal(await page.locator('#login-button').innerText(),'登录');
  await page.locator('#login-button').click();
  await page.locator('#login-user').fill('admin');
  await page.locator('#login-password').fill('browser-new');
  await page.locator('#login-form button[type="submit"]').click();
  await page.waitForFunction(()=>document.querySelector('#login-button').textContent.includes('admin'));
